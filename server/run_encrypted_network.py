...
def save_outputs(pid, outputs, zipfile=True):
    ...
    # zip results and delete intermediate values
    if zipfile:
        # add files to zip
...
def execute_network(pid, b_weights):
    ...
    inp_path = 'yaleface_enc_%s' % str_pid
    ...
    w_dic = enco_weights(weights_path, batched_weights=b_weights, enco=batch_encoder, name=process_name)
    ...
    save_outputs(pid=str_pid, outputs=out, zipfile=True)
    end_save = time.time()
